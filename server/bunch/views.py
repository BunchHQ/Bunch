...
from .permissions import IsBunchMember, IsBunchOwner, IsBunchAdmin
from .serializers import BunchSerializer, ChannelSerializer, MemberSerializer
...
class BunchViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, IsBunchOwner]
    lookup_field = "id"

    def get_queryset(self):
        all_bunches = Bunch.objects.all()
        user_memberships = self.request.user.bunch_memberships.all()
        queryset = Bunch.objects.filter(members__user=self.request.user).order_by(
            "-created_at"
        )
    ...
    def get_permissions(self):
        if self.action == "join":
            ...
            self.permission_classes = [permissions.IsAuthenticated, IsBunchMember]
        else:
            self.permission_classes = [permissions.IsAuthenticated, IsBunchOwner]
    ...
    def join(self, request, id=None):
        ...
        member = Member.objects.create(user=request.user, bunch=bunch, role="member")
...
class MemberViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, IsBunchMember]
    lookup_field = "id"

    def get_queryset(self):
        bunch_id = self.kwargs.get("bunch_id")
        return Member.objects.filter(bunch_id=bunch_id).order_by("-joined_at")

    def get_permissions(self):
        if self.action == "update_role":
            self.permission_classes = [permissions.IsAuthenticated, IsBunchAdmin]
        else:
            self.permission_classes = [permissions.IsAuthenticated, IsBunchMember]
        return super().get_permissions()
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(bunch=bunch, user=user, nickname=nickname, role=role)
