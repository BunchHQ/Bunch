...
class ReactionsTestCase(APITestCase):
    def setUp(self):
        ...
        self.channel = Channel.objects.create(
            bunch=self.bunch, name="general", type="text"
        )

        # Create members
        self.owner_member = Member.objects.get(
            bunch=self.bunch, user=self.owner, role=RoleChoices.OWNER
        )

        self.member_member = Member.objects.create(
            bunch=self.bunch, user=self.member, role=RoleChoices.MEMBER
        )
    ...
    def test_create_reaction(self):
        ...
        data = {"message_id": str(self.message.id), "emoji": "👍"}

        response = self.client.post(
            f"/api/v1/bunch/{self.bunch.id}/reactions/", data, format="json"
        )
    ...
    def test_toggle_reaction_add(self):
        ...
        data = {"message_id": str(self.message.id), "emoji": "❤️"}
    ...
    def test_reaction_counts_in_message_serializer(self):
        ...
        Reaction.objects.create(
            message=self.message, user=self.owner, emoji="👍"
        )
        Reaction.objects.create(
            message=self.message, user=self.member, emoji="❤️"
        )
    ...
    def test_invalid_emoji_rejected(self):
        ...
        data = {"message_id": str(self.message.id), "emoji": "not_an_emoji"}
