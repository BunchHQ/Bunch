...
class UserModelTest(TestCase):
    ...
    def setUp(self):
        User.objects.create_user(
            username="test", email="test@bunch.io", password="testpassword"
        )
        User.objects.create_superuser(
            username="root", email="root@bunch.io", password="rootpassword"
        )
    ...
    def test_creation(self):
        ...
        self.assertEqual(
            str(user), user.username, "String representation of user is not username"
        )

    def test_creation_with_invalid_username(self):
        with self.assertRaises(ValueError):
            User.objects.create_user(
                username="", email="test@bunch.io", password="testpassword"
            )

    def test_creation_with_invalid_email(self):
        with self.assertRaises(ValidationError, msg="Email is required."):
            User.objects.create_user(
                username="test2", email="", password="testpassword"
            )

    def test_username_is_unique(self):
        with self.assertRaises(IntegrityError):
            User.objects.create_user(
                username="test", email="test2@bunch.io", password="testpassword"
            )

    def test_email_is_unique(self):
        with self.assertRaises(IntegrityError):
            User.objects.create_user(
                username="test2", email="test@bunch.io", password="testpassword"
            )
    ...
    def test_default_theme_preference_is_system(self):
        ...
        self.assertEqual(
            ...
            "Default theme preference is not system",
        )
    ...
    def test_all_attributes(self):
        ...
        self.assertTrue(
            hasattr(user, "theme_preference"),
            "User does not have theme_preference attribute",
        )
        self.assertTrue(hasattr(user, "color"), "User does not have color attribute")
        self.assertTrue(
            hasattr(user, "pronoun"), "User does not have pronoun attribute"
        )
